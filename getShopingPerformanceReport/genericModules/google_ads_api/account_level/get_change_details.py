...
def main(client, customer_id):
    ...
    change_client_type_enum = client.get_type(
        "ChangeClientTypeEnum", version="v6"
    ).ChangeClientType
    ...
    try:
        ...
        for row in results:
            ...
            change_resource_type = event.change_resource_type
            change_resource_type = resource_type_enum.Name(
                change_resource_type
            )

            # print('operation event.resource_change_operation')
            resource_change_operation = event.resource_change_operation
            resource_change_operation = operation_type_enum.Name(
                resource_change_operation
            )


            client_type = event.client_type
            client_type =  change_client_type_enum.Name(
                event.client_type
            )

            print(
                f"On {event.change_date_time}, user {event.user_email} "
                f"used interface {client_type} to perform a(n) "
                f"{resource_change_operation} operation on a "
                f"{change_resource_type} with resource name "
