...
def main(client, customer_id, campaign_id):
    ...
    ad_group.name = "New API Testing %s" % uuid.uuid4()
    ad_group.status = client.get_type("AdGroupStatusEnum", version="v6").PAUSED
    
    # ad_group.status = 2 # ENABLED
    # ad_group.status = 3 # ENABLED

    # print('ad_group.status',ad_group.status)
    ad_group.campaign = campaign_service.campaign_path(customer_id, campaign_id)
    ad_group.type = client.get_type(
        "AdGroupTypeEnum", version="v6"
    ).SEARCH_STANDARD
    ad_group.cpc_bid_micros = 20000000
...
if __name__ == "__main__":
    ...
    if parser:
        ...
        main(google_ads_client, '4861039205', '9262642738')
